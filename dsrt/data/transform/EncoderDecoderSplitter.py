...
class EncoderDecoderSplitter:
    ...
    def encoder_decoder_split(self):
        ...
        pad = self.properties.pad
	        
	    # initialize encoder/decoder samples
        encoder_x = np.copy(self.dialogues[:, 0])
        decoder_x = np.zeros(encoder_x.shape)
        decoder_y = np.copy(self.dialogues[:, 1])
        ...
        return np.swapaxes(np.array([encoder_x, decoder_x, decoder_y]), 0, 1)
